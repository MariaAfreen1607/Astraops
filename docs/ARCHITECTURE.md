...
Mounts under `/briefs`. `GET /briefs/spaceweather` fetches the current space weather data, passes flares, CMEs, and storms to `brief_space_weather`, and additionally calls `estimate_drag` from `services/drag.py` to populate the optional `computed` field on the returned `Brief` with deterministic 72-hour altitude-loss figures for 210 km, 400 km, and 550 km. `GET /briefs/conjunction` screens the requested group, takes the closest event (index 0, since results are sorted by miss distance), and passes it to `brief_conjunction`. `GET /briefs/posture` fetches the tracked satellites for the requested group, the current space weather, and the full conjunction screen, then calls `brief_posture` to generate a one-line status indicator and justification sentence for the dashboard header. All three endpoints return a `Brief` model with `generated_at`, `model_used`, `subject`, `brief`, and an optional `computed` dict.
...
**[`services/drag.py`](../astraops-api/services/drag.py)**
A deterministic drag-decay estimation layer whose outputs are passed to Granite as a `COMPUTED` block; the language model is instructed to cite these figures rather than produce its own. `estimate_drag(cmes, storms)` selects the fastest CME and the highest observed Kp index from the current space weather data and drives two calculations. First, `_density_multiplier(kp)` returns `1.0 + 0.10 × Kp`, a linear relation anchored to the February 2022 event where Kp ≈ 5 corresponded to roughly 50 % above quiet-time density at 210 km. Second, `_decay_m_per_day(alt_km, multiplier)` computes orbital energy loss per revolution using the drag equation (`Cd = 2.2`, `A/m = 0.01 m²/kg`, representative of a 3U CubeSat) and the quiet-time `BASE_DENSITY` reference values at 210, 400, and 550 km; multiplied by revolutions per day this gives metres of altitude loss per day. The function returns a structured dict including 72-hour altitude loss at each reference altitude together with the quiet-time baseline so the brief can state the fractional increase. `format_for_prompt(est)` renders this dict as a plain-text `COMPUTED` block. When the fastest CME's speed is known, `_transit_hours` computes a Sun-Earth arrival time and `format_for_prompt` labels the CME as either en route or already arrived.

**[`services/granite.py`](../astraops-api/services/granite.py)**
The Granite reasoning layer. `_get_llm()` is `@lru_cache`-wrapped and returns `None` if Watsonx credentials are absent, disabling AI features gracefully. `_invoke(system, user)` sends a `[SystemMessage, HumanMessage]` pair to Granite 4 and retries up to three times with exponential back-off on transient errors (HTTP 429, consumption limit, timeout), returning a human-readable fallback string on permanent failure. `brief_space_weather(flares, cmes, storms)` formats up to eight events per type into a compact text payload and calls `_invoke` with `SPACE_WEATHER_SYSTEM`, a 200-word prompt that enforces orbit-regime separation (VLEO / LEO / MEO / GEO), prohibits invented figures, and requires IMPACT / SUBSYSTEMS / ACTION / CONFIDENCE sections. `brief_conjunction(event)` formats a single `ConjunctionEvent` and calls `_invoke` with `CONJUNCTION_SYSTEM`, a 180-word prompt that explains relative-velocity semantics and requires ASSESSMENT / SEVERITY / ACTION / CAVEAT sections. `brief_posture(sat_count, top_conjunction, flares, cmes, storms)` is a lightweight one-call summary: given the tracked satellite count, the closest screened conjunction, and current space weather, `POSTURE_SYSTEM` instructs Granite to return exactly two lines — a status word (`NOMINAL`, `ELEVATED`, or `ACTION REQUIRED`) followed by a single explanatory sentence. The status word maps to threshold rules enforced in the prompt: `ACTION REQUIRED` only if a conjunction is below 5 km or Kp ≥ 6.
